...
class InvisibleTool(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.key_input.setPlaceholderText("例如：Koodo、Chrome、Edge、知乎、晋江")
    ...
    def start_monitor(self):
        ...
        self.status_label.setText(f"正在监控：{text}")
        self.timer.start()
    ...
    def check_mouse_state(self):
        ...
        if mouse_in:
            ...
            self.status_label.setText(f"✅ {self.current_keyword} → 显示中")
        else:
            self.set_alpha(hwnd, 0)
            self.status_label.setText(f"🔒 {self.current_keyword} → 已隐身")
